...
For working with R we recommend using in [StatET](http://www.walware.de/goto/statet) if you are mainly using Eclipse. The editor suffices if you only run the scripts using `invesdwin-context-r`, so no complicated R setup with eclipse is needed, just install the plugin from the marketplace. For experimenting with R it might be interesting to use [RStudio](https://www.rstudio.com/) as a standalone development environment. It supports a nice variable viewer and has a nice integration of the R manuals, which helps a lot during R development.
